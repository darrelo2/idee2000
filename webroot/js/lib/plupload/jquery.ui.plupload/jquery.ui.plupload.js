...
/*global window:false, document:false, plupload:false, jQuery:false */
 ...
 //jQuery.noConflict();
jQuery(function(window, document, plupload, jQuery, undef) {
...
jQuery.widget("ui.plupload", {
	...
	_create: function() {
		...
		this.filelist = jQuery('.plupload_filelist_content', this.container).attr('id', id + '_filelist');
		
		// buttons
		this.browse_button = jQuery('.plupload_add', this.container).attr('id', id + '_browse');
		this.start_button = jQuery('.plupload_start', this.container).attr('id', id + '_start');
		this.stop_button = jQuery('.plupload_stop', this.container).attr('id', id + '_stop');
		...
		if (!this.options.buttons.browse) {
			...
			jQuery('#' + id + self.runtime + '_container').hide();
		}
		...
		uploader = this.uploader = uploaders[id] = new plupload.Uploader(jQuery.extend({ 
			container: id ,
		...
		uploader.bind('UploadProgress', function(up, file) {
			...
			jQuery('#' + file.id + ' .plupload_file_status', self.element).html(file.percent + '%');
	...
	_setOption: function(key, value) {
		...
		if (key == 'buttons' && typeof(value) == 'object') {	
			...
			if (!value.browse) {
				...
				jQuery('#' + self.id + self.runtime + '_container').hide();
			} else {
				self.browse_button.button('enable').show();
				jQuery('#' + self.id + self.runtime + '_container').show();
			}
	...
	_handleState: function() {
		...
		if (uploader.state === plupload.STARTED) {
			...
			jQuery(self.start_button).button('disable');
			...
			jQuery('.plupload_upload_status', self.element).text(
				_('Uploaded %d/%d files').replace('%d/%d', uploader.total.uploaded+'/'+uploader.files.length)
			);
			
			jQuery('.plupload_header_content', self.element).addClass('plupload_header_content_bw');
			...
			if (self.options.multiple_queues) {
				jQuery(self.start_button).button('enable');
						
				jQuery('.plupload_header_content', self.element).removeClass('plupload_header_content_bw');
			}
		...
		switch (file.status) {
			...
			case plupload.UPLOADING:
				...
				var scroller = jQuery('.plupload_scroll', this.container),
					...
					rowOffset = jQuery('#' + file.id).position().top + jQuery('#' + file.id).height();
	...
	_updateTotalProgress: function() {
		...
		jQuery('.plupload_total_status', this.element).html(uploader.total.percent + '%');
		
		jQuery('.plupload_upload_status', this.element).text(
			_('Uploaded %d/%d files').replace('%d/%d', uploader.total.uploaded+'/'+uploader.files.length)
		);
	},
	...
	_updateFileList: function() {
		...
		if (jQuery.ui.sortable && this.options.sortable) {
			jQuery('tbody', filelist).sortable('destroy');	
		}
		...
		jQuery.each(uploader.files, function(i, file) {
			...
			jQuery('#' + file.id + '.plupload_delete .ui-icon, #' + file.id + '.plupload_done .ui-icon')
				.click(function(e) {
		...
		jQuery('.plupload_total_file_size', self.element).html(plupload.formatSize(uploader.total.size));

		if (uploader.total.queued === 0) {
			jQuery('.ui-button-text', self.browse_button).text(_('Add Files'));
		} else {
			jQuery('.ui-button-text', self.browse_button).text(_('%d files queued').replace('%d', uploader.total.queued));
		}
		...
		if (!uploader.files.length && uploader.features.dragdrop && uploader.settings.dragdrop) {
			...
			jQuery('#' + id + '_filelist').append('<tr><td class="plupload_droptext">' + _("Drag files here.") + '</td></tr>');
		} else {
	...
	_enableRenaming: function() {
		...
		jQuery('.plupload_file_name span', this.filelist).live('click', function(e) {
			var targetSpan = jQuery(e.target), file, parts, name, ext = "";
	...
	_enableSortingList: function() {
		...
		if (jQuery('tbody tr', this.filelist).length < 2) {
			return;	
		}
		
		jQuery('tbody', this.filelist).sortable({
			containment: 'parent',
			...
			start: function(e, ui) {
				idxStart = jQuery('tr', this).index(ui.item);
			},
			
			stop: function(e, ui) {
				var i, length, idx, files = [], idxStop = jQuery('tr', this).index(ui.item);
